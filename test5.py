...
current_issues = {
    'TRX': {'issue': '', 'bet_type': '', 'amount': 0, 'step': 0}
}
...
class LotteryBot:
    def __init__(self, platform='TRX'):
        self.platform = platform
        self.base_url = API_ENDPOINTS.get(platform, API_ENDPOINTS['TRX'])
        
        # Set TRX platform headers
    ...
    async def get_current_issue(self):
        """Get current TRX game issue"""
        try:
            body = {
                "typeId": 2,  # TRX အတွက် 2 ပြောင်းမယ်
                "language": 0,
            ...
            logger.error(f"Get TRX issue error: {e}")
            return ""
    
    async def get_recent_results(self, count=5):
        """Get recent TRX game results"""
        try:
            body = {
                "pageNo": 1,
                "pageSize": count,
            ...
            logger.error(f"Get TRX results error: {e}")
            return []
...
def generate_signal_text(platform, issue, bet_type, amount, current_step, total_profit, confidence):
    """Generate signal message for channel - TRX Only"""
    signal_text = f"""
🔮777 BIG WIN WINGO 3 MIN🔮
💡 Issue: {issue}
🎲 Bet: {bet_type}
💰 Amount: {amount:,} K
    """
...
def generate_instant_result_text(platform, issue, bet_type, amount, result, profit_loss, current_step, total_profit, result_number):
    ...
    instant_message = f"""
{emoji} BET RESULT UPDATE {result_emoji}
...
async def send_signal_for_platform(context: ContextTypes.DEFAULT_TYPE, platform: str):
    ...
    try:
        ...
        logger.info(f"TRX Signal sent: {bet_type} {bet_amount}K (Step {current_step + 1})")
        return True
    ...
    try:
        ...
        if new_results and len(new_results) > 0:
            ...
            if result_issue == current_issue:
                ...
                logger.info(f"TRX Result: {result} (Profit: {profit_loss}, New Step: {new_step})")
...
async def check_TRX_results_continuously(context: ContextTypes.DEFAULT_TYPE):
    """TRX platform အတွက် ချက်ချင်း result စစ်ဆေးခြင်း"""
    try:
        platform = 'TRX'
        logger.info(f"Starting continuous result checking for TRX")
        ...
        while True:
            try:
                ...
                if current_issue_data['issue']:
                    ...
                    if new_results and len(new_results) > 0:
                        ...
                        if result_issue == current_issue_data['issue']:
                            await process_TRX_result(context, platform, current_issue_data, result_issue, result_number)
                ...
                logger.error(f"Error in continuous TRX result check: {e}")
                await asyncio.sleep(5)
                
    except Exception as e:
        logger.error(f"Continuous TRX result checking stopped: {e}")

async def process_TRX_result(context: ContextTypes.DEFAULT_TYPE, platform: str, issue_data: dict, result_issue: str, result_number: str):
    """TRX result ကို process လုပ်ခြင်း"""
    try:
        current_issue = issue_data['issue']
        bet_type = issue_data['bet_type']
        bet_amount = issue_data['amount']
        ...
        logger.info(f"TRX Result processed: {result} (Profit: {profit_loss}, New Step: {new_step})")
        
        # WinLoss ပြပြီးရင် နောက် Issue ထိုးဖို့ signal return True
        return True
        
    except Exception as e:
        logger.error(f"Error processing TRX result: {e}")
        return False

async def start_auto_signal(context: ContextTypes.DEFAULT_TYPE):
    """Start auto signal service for TRX platform only"""
    try:
        logger.info("TRX Auto signal service started")
        
        # Initial delay to let bot fully start
        await asyncio.sleep(10)
        
        # TRX platform အတွက် continuous result checking စမယ်
        asyncio.create_task(check_TRX_results_continuously(context))
        logger.info("TRX continuous result checking started")
        
        # ပထမဆုံး signal ကိုချက်ချင်းပို့မယ်
        await send_signal_for_platform(context, 'TRX')
        
        while True:
            try:
                start_time = datetime.now()
                logger.info(f"Starting new TRX signal cycle at {start_time.strftime('%H:%M:%S')}")
                
                # TRX platform အတွက်သာ signal ပို့မယ်
                platform = 'TRX'
                ...
                if not current_issue_data['issue']:
                    ...
                    if signal_sent:
                        logger.info("TRX signal sent successfully")
                    else:
                        logger.error("Failed to send TRX signal")
                ...
                if wait_time > 0:
                    logger.info(f"Waiting {wait_time:.1f} seconds for next TRX signal")
                    await asyncio.sleep(wait_time)
                else:
                    logger.info("Starting next TRX signal immediately")
                    
            except Exception as e:
                logger.error(f"Error in TRX signal cycle: {e}")
                await asyncio.sleep(10)
                
    except Exception as e:
        logger.error(f"TRX Auto signal service stopped: {e}")
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not has_joined:
        welcome_text = f"""
🎰 Welcome to WINGO 3 MIN Auto Signal Bot 🎯
...
• 📊 Get real-time WINGO 3 MIN LOTTERY signals
...
Please join our channel below and then click ✅ I've Joined to verify.
    ...
    welcome_text = f"""
🎰 WINGO 3 MIN Auto Signal Bot 🎯

Welcome {user.first_name}!

🤖 WINGO 3 MIN Automatic Signal Features:
• 📊 WINGO 3 MIN LOTTERY Signals Only
...
🚀 Current Mode: WINGO 3 MIN platform signals with instant WinLoss and immediate next issue!
        """
...
async def status_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Check current status for TRX platform"""
    try:
        platform = 'TRX'
        sequence_data = get_platform_sequence(platform)
        current_step = sequence_data['current_step']
        ...
        status_text = f"""
📊 WINGO 3 MIN LOTTERY Bot Status
...
async def force_signal_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Force send signal for TRX platform immediately"""
    try:
        await update.message.reply_text("🔄 Forcing immediate TRX signal...")
        
        platform = 'TRX'
        success = await send_signal_for_platform(context, platform)
        
        if success:
            await update.message.reply_text("✅ TRX signal sent successfully!")
        else:
            await update.message.reply_text("❌ Failed to send TRX signal.")
...
def main():
    ...
    logger.info("TRX Auto Signal Bot starting...")
    print("🤖 TRX Auto Signal Bot is running...")
    print("📢 Auto Signal System: ENABLED")
    print(f"📊 Signal Channel: {SIGNAL_CHANNEL_USERNAME}")
    print("🎯 Platform: TRX LOTTERY ONLY")
