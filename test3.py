...
# Bot configuration
BOT_TOKEN = "8539481129:AAH7zucGxvkcOL4wHrqAYZ_y30-jMP50ZOY"

# Channel configuration - Auto Signal ချမယ့် Channel
SIGNAL_CHANNEL_USERNAME = "@ck_data_drift"  # Your signal channel
...
def generate_signal_text(platform, issue, bet_type, amount, current_step, total_profit, confidence):
    """Generate signal message for channel - CK Only"""
    # WainLoss ကိုထည့်သွင်းခြင်း
    wain_loss = "WainLoss" if bet_type == "BIG" else "WainLoss"
...
async def send_signal_for_platform(context: ContextTypes.DEFAULT_TYPE, platform: str):
    ...
    try:
        ...
        logger.info(f"CK Signal sent: {bet_type} {bet_amount}K (Step {current_step + 1})")
        return True
    ...
    try:
        ...
        if new_results and len(new_results) > 0:
            ...
            if result_issue == current_issue:
                ...
                logger.info(f"CK Result: {result} (Profit: {profit_loss}, New Step: {new_step})")
...
async def check_ck_results_continuously(context: ContextTypes.DEFAULT_TYPE):
    """CK platform အတွက် ချက်ချင်း result စစ်ဆေးခြင်း"""
    try:
        platform = 'ck'
        logger.info(f"Starting continuous result checking for CK")
        ...
        while True:
            try:
                ...
                if current_issue_data['issue']:
                    ...
                    if new_results and len(new_results) > 0:
                        ...
                        if result_issue == current_issue_data['issue']:
                            await process_ck_result(context, platform, current_issue_data, result_issue, result_number)
                ...
                logger.error(f"Error in continuous CK result check: {e}")
                await asyncio.sleep(5)
                
    except Exception as e:
        logger.error(f"Continuous CK result checking stopped: {e}")

async def process_ck_result(context: ContextTypes.DEFAULT_TYPE, platform: str, issue_data: dict, result_issue: str, result_number: str):
    """CK result ကို process လုပ်ခြင်း"""
    try:
        current_issue = issue_data['issue']
        bet_type = issue_data['bet_type']
        bet_amount = issue_data['amount']
        ...
        logger.info(f"CK Result processed: {result} (Profit: {profit_loss}, New Step: {new_step})")
        
        # WinLoss ပြပြီးရင် နောက် Issue ထိုးဖို့ signal return True
        return True
        
    except Exception as e:
        logger.error(f"Error processing CK result: {e}")
        return False

async def start_auto_signal(context: ContextTypes.DEFAULT_TYPE):
    """Start auto signal service for CK platform only"""
    try:
        logger.info("CK Auto signal service started")
        
        # Initial delay to let bot fully start
        await asyncio.sleep(10)
        
        # CK platform အတွက် continuous result checking စမယ်
        asyncio.create_task(check_ck_results_continuously(context))
        logger.info("CK continuous result checking started")
        
        # ပထမဆုံး signal ကိုချက်ချင်းပို့မယ်
        await send_signal_for_platform(context, 'ck')
        
        while True:
            try:
                start_time = datetime.now()
                logger.info(f"Starting new CK signal cycle at {start_time.strftime('%H:%M:%S')}")
                
                # CK platform အတွက်သာ signal ပို့မယ်
                platform = 'ck'
                ...
                if not current_issue_data['issue']:
                    ...
                    if signal_sent:
                        logger.info("CK signal sent successfully")
                    else:
                        logger.error("Failed to send CK signal")
                ...
                if wait_time > 0:
                    logger.info(f"Waiting {wait_time:.1f} seconds for next CK signal")
                    await asyncio.sleep(wait_time)
                else:
                    logger.info("Starting next CK signal immediately")
                    
            except Exception as e:
                logger.error(f"Error in CK signal cycle: {e}")
                await asyncio.sleep(10)
                
    except Exception as e:
        logger.error(f"CK Auto signal service stopped: {e}")
...
• 📊 Get real-time CK LOTTERY signals
...
🤖 **CK Automatic Signal Features:**
• 📊 CK LOTTERY Signals Only
...
🚀 **Current Mode:** CK platform signals with instant WinLoss and immediate next issue!
        """
...
async def status_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Check current status for CK platform"""
    try:
        platform = 'ck'
        sequence_data = get_platform_sequence(platform)
        current_step = sequence_data['current_step']
...
async def force_signal_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Force send signal for CK platform immediately"""
    try:
        await update.message.reply_text("🔄 Forcing immediate CK signal...")
        
        platform = 'ck'
        success = await send_signal_for_platform(context, platform)
        
        if success:
            await update.message.reply_text("✅ CK signal sent successfully!")
        else:
            await update.message.reply_text("❌ Failed to send CK signal.")
...
def main():
    ...
    logger.info("CK Auto Signal Bot starting...")
    print("🤖 CK Auto Signal Bot is running...")
    print("📢 Auto Signal System: ENABLED")
    print(f"📊 Signal Channel: {SIGNAL_CHANNEL_USERNAME}")
    print("🎯 Platform: CK LOTTERY ONLY")
